...
<div class="infographic-panel relative overflow-hidden rounded-3xl border border-gray-200 p-5 sm:p-7" style="background:linear-gradient(180deg,#FFF8EC 0%,#ffffff 100%);">
  <div class="flex items-center justify-between mb-5">
    <div>
      <p class="text-[11px] font-semibold uppercase tracking-[0.18em] mb-1" style="color:#E8A317;">Journey Pipelines</p>
  ...
  <svg viewBox="0 0 720 260" class="w-full h-auto" role="img" aria-label="Kanban journey pipeline with Lead, Qualified, Proposal, and Won stages">
    ...
    @php
      $cols = [
        ...
        [540, 'Won', '3', '#E8A317', [['Leo T.', 'Paid ✓']]],
      ];
    @endphp

    {{-- Flow arrows between columns --}}
    <path d="M175 40 H195" stroke="rgba(232,163,23,0.4)" stroke-width="2" marker-end="url(#jpArrow)"/>
    <path d="M345 40 H365" stroke="rgba(232,163,23,0.4)" stroke-width="2"/>
    <path d="M515 40 H535" stroke="rgba(232,163,23,0.4)" stroke-width="2"/>
    <defs>
      <marker id="jpArrow" markerWidth="6" markerHeight="6" refX="5" refY="3" orient="auto">
        <path d="M0,0 L6,3 L0,6 Z" fill="#E8A317"/>
      </marker>
    </defs>
    ...
    @foreach($cols as [$x, $name, $count, $color, $cards])
      <g>
        ...
        <text x="{{ $x + 30 }}" y="42" fill="#475569" font-size="12" font-weight="700" font-family="Fraunces,Georgia,serif">{{ $name }}</text>
        ...
        @foreach($cards as $i => [$who, $note])
          <rect x="{{ $x + 10 }}" y="{{ 60 + $i * 70 }}" width="130" height="58" rx="10" fill="#FFF8EC" stroke="rgba(7,94,84,0.1)"/>
    ...
    <rect x="540" y="200" width="150" height="32" rx="8" fill="rgba(232,163,23,0.1)" stroke="rgba(232,163,23,0.3)"/>
    <text x="615" y="220" text-anchor="middle" fill="#E8A317" font-size="10" font-weight="600" font-family="DM Sans,sans-serif">Campaign fired →</text>
  </svg>
</div>
