...
class FaviconBrandingTest extends TestCase
{
    ...
    public function test_landing_page_declares_unganisha_favicon_links(): void
    {
        ...
        $this->assertStringContainsString('/landing/unganisha/android-chrome-192x192.png', $html);
        $this->assertStringContainsString('/landing/unganisha/favicon-32x32.png', $html);
        $this->assertStringContainsString('/landing/unganisha/favicon-16x16.png', $html);
        $this->assertStringContainsString('/landing/unganisha/favicon.ico', $html);
        $this->assertStringContainsString('/landing/unganisha/apple-touch-icon.png', $html);
        $this->assertStringContainsString('/landing/unganisha/site.webmanifest', $html);
        $this->assertStringContainsString('/landing/unganisha/mark.svg', $html);
    ...
    public function test_login_page_does_not_use_unganisha_landing_assets(): void
    {
        $response = $this->get('/login');

        $response->assertOk();
        $this->assertStringNotContainsString('/landing/unganisha/', $response->getContent());
        $this->assertStringNotContainsString('Unganisha', $response->getContent());
    }
    ...
    public function test_web_manifest_names_unganisha(): void
    {
        $manifest = json_decode(file_get_contents(public_path('site.webmanifest')), true);

        $this->assertSame('Unganisha', $manifest['name']);
        $this->assertSame('Unganisha', $manifest['short_name']);
        $this->assertNotEmpty($manifest['icons']);
        $this->assertSame('#12263A', $manifest['theme_color']);
    }
