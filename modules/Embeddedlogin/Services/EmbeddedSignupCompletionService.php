...
class EmbeddedSignupCompletionService
{
    ...
    public function complete(User $user, string $code, EmbeddedSignupSession $session): array
    {
        ...
        $this->persistWhatsappCompanyConfig($company, $phoneId, $wabaId, $accessToken);
    ...
    private function provisionMetaMessagingChannels(
        ...
        $this->subscribePageWebhooks($accessToken, $pageId);
        ...
        if ($session->instagramAccountId) {
            ...
            $company->setConfig('instagram_account_id', $session->instagramAccountId);
            $company->setConfig('instagram_page_access_token', $accessToken);
        }
        ...
        $company->setConfig('messenger_page_access_token', $accessToken);
