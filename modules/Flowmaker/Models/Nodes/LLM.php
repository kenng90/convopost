...
class LLM extends Node
{
    ...
    public function process($message, $data)
    {
        ...
        try {
            ...
            // Resolve OpenRouter API key (company BYOK or platform managed AI)
            $flow = Flow::find($this->flow_id);
            Log::info('Flow', ['flow' => $flow]);
            $company = $flow->company;
            Log::info('Company', ['company' => $company]);

            $managedAi = app(ManagedAiService::class);
            $keyInfo = $managedAi->resolveOpenRouterKey($company);
            $apiKey = $keyInfo['key'];
            $shouldMeter = $keyInfo['should_meter'];
            $llmAction = 'ai_llm_reply';
            $llmCost = $managedAi->actionCost($llmAction);

            Log::info('API Key resolution', ['source' => $keyInfo['source'], 'should_meter' => $shouldMeter]);

            if (! $apiKey) {
                throw new \Exception('OpenRouter API key not configured');
            }

            if ($shouldMeter && ! $managedAi->canConsume($company, $llmCost)) {
                throw new \Exception('Managed AI credits exhausted for LLM node');
            }
            ...
            if ($shouldMeter) {
                $managedAi->consume($company, $llmCost, $llmAction, [
                    'model' => $model,
                    'node' => 'llm',
                ]);
            }
