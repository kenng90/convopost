...
<div class="infographic-panel relative overflow-hidden rounded-3xl border border-gray-200 p-6 sm:p-8" style="background:linear-gradient(180deg,#FFF8EC 0%,#ffffff 100%);">
  <div class="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3 mb-6">
    <div>
      <p class="text-[11px] font-semibold uppercase tracking-[0.18em] mb-2" style="color:#0E8A7A;">Commerce loop</p>
  ...
  <svg viewBox="0 0 960 220" class="w-full h-auto hidden md:block" role="img" aria-label="Commerce journey from discovery through payment and retention">
    <defs>
      <marker id="arrowGreen" markerWidth="8" markerHeight="8" refX="6" refY="4" orient="auto">
        <path d="M0,0 L8,4 L0,8 Z" fill="#0E8A7A"/>
      </marker>
      <linearGradient id="flowGrad" x1="0%" y1="0%" x2="100%" y2="0%">
        <stop offset="0%" stop-color="#0E8A7A" stop-opacity="0.2"/>
        <stop offset="100%" stop-color="#0E8A7A" stop-opacity="0.8"/>
      </linearGradient>
    </defs>
    ...
    @foreach($steps as [$cx, $num, $title, $sub])
      <g>
        <circle cx="{{ $cx }}" cy="110" r="28" fill="#ffffff" stroke="#0E8A7A" stroke-width="2"/>
        <text x="{{ $cx }}" y="114" text-anchor="middle" fill="#0E8A7A" font-size="12" font-weight="700" font-family="Outfit,Plus Jakarta Sans,sans-serif">{{ $num }}</text>
        <text x="{{ $cx }}" y="160" text-anchor="middle" fill="#0f172a" font-size="13" font-weight="700" font-family="Outfit,Plus Jakarta Sans,sans-serif">{{ $title }}</text>
  ...
  <div class="md:hidden space-y-3">
    @foreach([
      ...
      <div class="flex items-center gap-3 rounded-2xl border border-gray-200 px-4 py-3" style="background:rgba(14,138,122,0.08);">
        <span class="font-display text-sm font-800" style="color:#0E8A7A;">{{ $num }}</span>
        <div>
