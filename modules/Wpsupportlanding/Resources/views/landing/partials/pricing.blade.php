<section id="pricing" class="relative py-20 bg-gray-100 ">
    <div class="relative px-10 mx-auto max-w-7xl xl:px-16">
        <div class="max-w-3xl mx-auto mb-12 text-center lg:mb-20">
            <span class="text-sm font-semibold text-green-500">{{ __('wpsupport.our_subscription_plans') }}</span>
            <h2 class="mt-3 mb-10 text-4xl font-bold font-heading">{{ __('wpsupport.simple_flexible_pricing') }}</h2>
            <p class="mb-16 text-xl text-gray-500">{{ __('wpsupport.subscription_plans_description') }}</p>
        </div>
                ...
                <div class="w-full px-4 lg:w-5/12 2xl:w-4/12">
                    <div class="h-full p-12 text-center bg-white rounded-xl">
                        <span class="inline-block px-3 py-1 mb-4 rounded bg-blue-50">
                            <h3 class="text-xs font-semibold text-blue-500">{{ __('wpsupport.plan_title',['plan'=>$plan->name]) }}</h3>
                        </span>
                        <p class="mb-6 text-gray-500 lg:mb-12">{{ __('wpsupport.plan_subscribe_info',['plan'=>$plan->name])  }}</p>
                        ...
                        <a class="block py-4 mb-4 text-sm font-medium leading-normal text-center text-white transition duration-200 bg-blue-400 rounded hover:bg-blue-300" href="{{ route('register') }}">{{ __('wpsupport.start_now')}}</a>
                        @endif
                        <p class="text-xs text-gray-500">
                            {{ __('wpsupport.no_contracts')}}
                        </p>
                    </div>
                </div>
            </div>
                ...
                <div class="w-full px-4 lg:w-5/12 2xl:w-4/12">
                    <div class="h-full p-12 text-center bg-white rounded-xl">
                        <span class="inline-block px-3 py-1 mb-4 rounded bg-green-50">
                            <h3 class="text-xs font-semibold text-green-500">{{ __('wpsupport.plan_title',['plan'=>$plan->name]) }}</h3>
                        </span>
                        <p class="mb-6 text-gray-500 lg:mb-12">{{ __('wpsupport.plan_subscribe_info',['plan'=>$plan->name])  }}</p>
                        <div class="flex justify-center mb-12">
                            <span class="self-start inline-block mr-1 text-xl font-semibold text-gray-500">{{ config('money')[strtoupper(config('settings.cashier_currency'))]['symbol'] }}</span>
                        ...
                        @if (!config('settings.disable_registration_page',false))
                        <a class="block py-4 mb-4 text-sm font-medium leading-normal text-center text-white transition duration-200 bg-green-400 rounded hover:bg-green-300" href="{{ route('register') }}">{{ __('wpsupport.start_now')}}</a>
                        @endif
                        <p class="text-xs text-gray-500">
                            {{ __('wpsupport.no_contracts')}}
                        </p>
