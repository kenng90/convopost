...
<div class="infographic-panel relative overflow-hidden rounded-3xl border border-gray-200 p-5 sm:p-7" style="background:linear-gradient(180deg,#FFF8EC 0%,#ffffff 100%);">
  <div class="mb-5">
    <p class="text-[11px] font-semibold uppercase tracking-[0.18em] mb-1" style="color:#0E8A7A;">Booking lifecycle</p>
    <p class="text-base text-gray-600">The Bookings app inside Unganisha — widget to reminder in one loop</p>
  </div>
  ...
  <svg viewBox="0 0 680 200" class="w-full h-auto hidden sm:block" role="img" aria-label="Booking flow from service setup through WhatsApp reminders">
    <path d="M40 100 H640" stroke="rgba(14,138,122,0.25)" stroke-width="2" stroke-dasharray="5 7" fill="none"/>
    @php
      $bsteps = [
    ...
    @foreach($bsteps as $i => [$cx, $t, $s])
      <g>
        <circle cx="{{ $cx }}" cy="100" r="22" fill="#ffffff" stroke="#0E8A7A" stroke-width="2"/>
        <text x="{{ $cx }}" y="104" text-anchor="middle" fill="#0E8A7A" font-size="11" font-weight="700" font-family="Outfit,Plus Jakarta Sans,sans-serif">{{ $i + 1 }}</text>
        <text x="{{ $cx }}" y="148" text-anchor="middle" fill="#0f172a" font-size="12" font-weight="700" font-family="Outfit,Plus Jakarta Sans,sans-serif">{{ $t }}</text>
  ...
  <div class="sm:hidden grid grid-cols-1 gap-2">
    @foreach([
      ...
      <div class="flex items-center gap-3 rounded-xl border border-gray-200 px-3 py-2.5" style="background:rgba(14,138,122,0.08);">
