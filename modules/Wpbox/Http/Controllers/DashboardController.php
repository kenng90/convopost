...
class DashboardController extends Controller
{
    public function index()
    {
        ...
        if ($user->hasRole('owner') || $user->hasRole('staff')) {
            ...
            if ($this->getCompany() && $activation->shouldRedirectUserToActivation($user, $this->getCompany())) {
        ...
        if ($user->isOrganizationManager() && app(OrgAuthorization::class)->canAccessModule($user, 'wpbox')) {
            return $this->asCompany();
        }
    }
    ...
    public function setup()
    {
        ...
        if ($company->id == 1 && config('settings.is_demo', false)) {
            return redirect(route('chat.index'))->withStatus(__('This view is not allowed for the Demo company. Please create your account, so you can see the WhatsApp Cloud Setup view.'));
        }
    ...
    public function savesetup(Request $request)
    {
        ...
        $activation = app(ActivationService::class);
        if ($activation->shouldRedirectUserToActivation(auth()->user(), $company)) {
            return redirect(route('activation.index'))->withStatus(__('Settings updated'));
        }
    ...
    public function asCompany()
    {
        ...
        //Don't show wpbox dashboard if we are not in wpbox
        if (config('settings.app_code_name') != 'wpbox') {
