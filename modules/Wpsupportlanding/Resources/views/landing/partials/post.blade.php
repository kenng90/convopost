...
<article class="max-w-3xl mx-auto">
    <a href="{{ url('/blog') }}" class="inline-flex items-center gap-2 text-sm text-gray-600 hover:text-ink transition-colors mb-8">
    ...
    <header class="mb-8">
        ...
        <h1 class="font-display text-3xl sm:text-4xl lg:text-5xl font-800 text-ink leading-tight mb-4">{{ $post->title }}</h1>
    ...
    @if($post->featured_image)
        <div class="mb-10 rounded-2xl overflow-hidden border border-indigo/10">
            <img src="{{ $post->featured_image }}" alt="{{ $post->title }}" class="w-full h-auto max-h-[420px] object-cover">
        </div>
    @endif

    @if($post->excerpt)
        <p class="text-lg text-gray-600 mb-8 leading-relaxed border-l-2 pl-5" style="border-color:#E8A317;">{{ $post->excerpt }}</p>
    @endif
