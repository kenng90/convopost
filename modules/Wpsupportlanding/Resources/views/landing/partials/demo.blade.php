    ...
    <div class="relative px-12 mx-auto max-w-7xl">
        ...
        <div class="relative grid items-center grid-cols-1 gap-10 p-12 overflow-hidden shadow-2xl md:p-16 lg:p-20 xl:p-24 bg-gradient-to-r from-green-400 to-green-500 rounded-2xl md:grid-cols-5 lg:gap-0" data-primary="green-400" data-rounded="rounded-2xl" data-rounded-max="rounded-full">
            ...
            <div class="relative z-30 col-span-1 md:col-span-3">
                <h2 class="mb-1 text-3xl font-bold leading-tight text-white md:mb-3 md:text-3xl lg:text-4xl xl:text-5xl">{{ __('wpsupport.test_chat_system') }}</h2>
                <p class="max-w-sm my-6 text-base font-normal text-green-200 xl:max-w-lg lg:pr-0 pr-7 lg:text-xl" data-primary="green-400">{{ __('wpsupport.scan_qr_description') }}</p>
                <a href="{{ __('wpsupport.demo_chat_link' )}}" class="flex items-center justify-center w-full px-4 py-2 text-base font-medium leading-6 text-green-500 whitespace-no-wrap bg-white border-2 border-transparent rounded-full shadow-sm md:w-auto md:inline-flex hover:bg-transparent hover:text-white hover:border-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-600" data-primary="green-400" data-rounded="rounded-full">
                   {{ __('wpsupport.start_demo_chat') }}
                </a>
                <a target="_blank" href="{{ __('wpsupport.demo_knowledge_base_link' )}}" class="flex items-center justify-center w-full px-4 py-2 text-base font-medium leading-6 text-green-500 whitespace-no-wrap bg-white border-2 border-transparent rounded-full shadow-sm md:w-auto md:inline-flex hover:bg-transparent hover:text-white hover:border-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-600" data-primary="green-400" data-rounded="rounded-full">
                   {{ __('wpsupport.see_demo_knowledge_base') }}
                </a>
            </div>
            <a href="{{ __('wpsupport.demo_chat_link' )}}" class="flex items-center h-full col-span-1 md:col-span-2 mr-10">
                <img style="max-height:320px" src="{{ asset('default/wpbox/qr.png')}}" class="pr-10 relative top-0 right-2 object-cover max-h-md w-full -mr-1 rounded md:max-w-sm md:w-auto max-w-none lg:absolute lg:max-w-md xl:max-w-lg lg:mt-9 shadow-sm" alt="Dashboard">
            </a>
        </div>
    </div>
</section>