<section id="testimonials" class="w-full bg-gray-50 sm:py-16 py-12 md:py-20 relative " >
    <div class="max-w-7xl mx-auto px-10">
        <div class="flex flex-col mb-7 sm:mb-20 items-start md:items-center justify-center">
            <p class="sm:tracking-widest sm:text-base text-sm uppercase font-medium text-gray-500">{{ __('wpsupport.what_people_say') }}</p>
            <h2 class="text-gray-900 mt-2 text-2xl sm:text-4xl tracking-tight font-bold sm:font-extrabold md:text-5xl dark:text-white">{{ __('wpsupport.dont_take_our_word') }}</h2>
            <div class="mt-1 md:mt-4 items-center flex text-blue-600">
                ...
                <p class="text-xl md:text-4xl tracking-tight sm:font-bold sm:text-3xl">{{ __('wpsupport.view_testimonials') }}</p>
            </div>
        </div>
        <div class="grid grid-cols-4 sm:grid-cols-8 lg:grid-cols-12 gap-6 w-full">
            <div class="col-span-4 space-y-6">
                @foreach ($testimonials as $key => $testimonial)
                    <!-- if mod of $key -->
                    @if ($key % 3 == 0)
                        @include('wpsupportlanding::landing.partials.testimonial') 
                    @endif
                @endforeach
            </div>
