...
>
    ...
    <div class="bg-yellow-100 border border-yellow-400 p-4 m-4 rounded text-sm">
        <strong>Debug Info:</strong><br>
        flowId from Blade = <strong>{{ $flowId ?? 'NULL' }}</strong><br>
        View name used = livewire.flows-builder
    </div>
    {{-- ── Notification Toast ──────────────────────────────────────────────── --}}
    <div
    ...
    {{-- ── Main Builder (shown after load) ────────────────────────────────── --}}
    <div x-show="!loading" class="flex flex-1 overflow-hidden">

        {{-- ── Left: Screen Navigator ──────────────────────────────────────── --}}
        <div class="w-56 bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 flex flex-col flex-shrink-0">
        ...
        <div class="w-80 bg-gray-50 dark:bg-gray-900 border-r border-gray-200 dark:border-gray-700 flex flex-col flex-shrink-0">
...
<script>
console.log('=== INLINE SCRIPT TEST - Should appear ===');
    console.log('flowId from Blade:', {{ $flowId ?? 'null' }});
function flowBuilder(initialFlowId) {
    return {
        ...
        // ── Meta type map ─────────────────────────────────────────────────────
        metaTypeMap: {
            text: ['string', 'example text'], textarea: ['string', 'example text'],
            radio: ['string', 'option1'], select: ['string', 'option1'],
            chips: ['string', 'option1'], date: ['string', '2026-01-01'],
            checkbox: ['array', ['option1']], optin: ['boolean', true],
        },

        defaultLabels: {
            text: 'Text Input', textarea: 'Multi-line Text', radio: 'Select One',
            checkbox: 'Select Multiple', select: 'Dropdown', date: 'Select Date',
            chips: 'Quick Select', optin: 'I agree to terms', heading: 'Page Heading',
            subheading: 'Subheading', body: 'Body text', caption: 'Caption',
            richtext: 'Rich text', image: 'Image', image_carousel: 'Image Carousel',
            media_upload: 'Upload File', embedded_link: 'Open Link',
            footer: 'Continue', button: 'Submit',
        },

        typeIcons: {
            heading: 'T', subheading: 'T', body: '¶', caption: '¶', richtext: '¶',
            text: '✏', textarea: '✏', radio: '◎', chips: '◎', checkbox: '☑',
            select: '▾', date: '📅', optin: '✓', image: '🖼', image_carousel: '🖼',
            media_upload: '📎', embedded_link: '🔗', footer: '▬', button: '▬', navigate: '▬',
        },
        ...
        //         this.flowCategory    = f.category || 'OTHER';
        //         this.metaFlowId      = f.meta_flow_id || null;
        //         this.screens         = f.screens || [];
        //         if (this.screens.length > 0) {
        //             this.selectedScreenId = this.screens[0].id;
        //         }
        //         this.isDirty = false;
        //     } catch(e) {
        //         this.notify('Failed to load flow: ' + e.message, 'error');
        //     } finally {
        //         this.loading = false;
        //     }
        // },
        async loadFlow() {
            ...
            try {
                console.log(`Fetching flow from API: /api/flow-builder/${this.flowId}`);
                ...
                this.isDirty = false;
                this.notify('Flow loaded successfully', 'success');
                
            } catch(e) {
                console.error('Load flow error:', e);
                this.notify('Failed to load flow: ' + (e.message || 'Unknown error'), 'error');
        ...
        addScreen() {
            ...
            this.screens.push({ id, title: 'Screen ' + count, fields: [] });
        ...
        updateScreenTitle(id, title) {
            ...
            if (screen) { screen.title = title; this.isDirty = true; }
        },
...
console.log('=== Alpine script block LOADED successfully ===');
// Force initialization after everything is ready
window.addEventListener('load', () => {
    console.log('Window load event - forcing Alpine component init');
    const root = document.querySelector('[x-data]');
    if (root && typeof Alpine !== 'undefined') {
        Alpine.initTree(root);
        console.log('Alpine.initTree called on root element');
    }
});
